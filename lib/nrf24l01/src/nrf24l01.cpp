...
NRF24L01::OperationMode NRF24L01::mode(void) { return _mode; }
...
void NRF24L01::set_payload_size(RxAddressPipe rx_addr_pipe,
...
uint8_t NRF24L01::payload_size(void) { return _payload_size; }
...
void NRF24L01::attach_transmitting_payload(RxAddressPipe rx_address_pipe,
                                           uint8_t *hw_addr,
                                           uint8_t payload_size) {
    switch (rx_address_pipe) {
        case RxAddressPipe::RX_ADDR_P0:
            // set rx addr to pipe 0
            spi_write_register(RegisterAddress::REG_RX_ADDR_P0,
                               (const char *)hw_addr, 5);
            break;
        case RxAddressPipe::RX_ADDR_P1:
            // set rx addr to pipe 1
            spi_write_register(RegisterAddress::REG_RX_ADDR_P1,
                               (const char *)hw_addr, 5);
            break;
        case RxAddressPipe::RX_ADDR_P2:
            // set rx addr to pipe 2
            spi_write_register(RegisterAddress::REG_RX_ADDR_P2,
                               (const char *)hw_addr, 5);
            break;
        case RxAddressPipe::RX_ADDR_P3:
            // set rx addr to pipe 3
            spi_write_register(RegisterAddress::REG_RX_ADDR_P3,
                               (const char *)hw_addr, 5);
            break;
        case RxAddressPipe::RX_ADDR_P4:
            // set rx addr to pipe 4
            spi_write_register(RegisterAddress::REG_RX_ADDR_P4,
                               (const char *)hw_addr, 5);
            break;
        case RxAddressPipe::RX_ADDR_P5:
            // set rx addr to pipe 5
            spi_write_register(RegisterAddress::REG_RX_ADDR_P5,
                               (const char *)hw_addr, 5);
            break;
    }
...
void NRF24L01::attach_receive_payload(RxAddressPipe rx_address_pipe,
                                      uint8_t *hw_addr, uint8_t payload_size) {
    // set payload
    set_payload_size(rx_address_pipe, payload_size);
...
void NRF24L01::attach_receive_address_to_pipe(RxAddressPipe rx_address_pipe,
                                              uint8_t *hw_rx_addr) {
    switch (rx_address_pipe) {
        case RxAddressPipe::RX_ADDR_P0:
            // set rx addr to pipe 0
            spi_write_register(RegisterAddress::REG_RX_ADDR_P0,
                               (const char *)hw_rx_addr, 5);
            // enable rx addr
            spi_write_register(RegisterAddress::REG_EN_RXADDR, 0x01);
            break;
        case RxAddressPipe::RX_ADDR_P1:
...
void NRF24L01::set_rf_frequency(uint16_t rf_frequency) {
    ...
    if ((rf_frequency < MIN_RF_FREQUENCY) ||
        (rf_frequency > MAX_RF_FREQUENCY)) {
 ...
 * transport layer
 ***************************************************************************/
void NRF24L01::spi_select(void) { _com_cs = 0; }

void NRF24L01::spi_deselect(void) { _com_cs = 1; }
...
void NRF24L01::spi_read_payload(char *buffer, uint8_t length) {
    ...
    while (length--) {
        *buffer++ =
            _spi->write(static_cast<uint8_t>(RegisterOperation::OP_NOP));
    }
    spi_deselect();
#else
    static char reg;
...
void NRF24L01::spi_write_register(RegisterAddress register_address,
                                  uint8_t value) {
#ifdef _SPI_API_WITHOUT_CS_
    spi_select();
    _spi->write((static_cast<uint8_t>(register_address) |
                 static_cast<uint8_t>(RegisterOperation::OP_WRITE)));
    _spi->write(value);
    spi_deselect();
#else
    static char data[2];
    ...
    data[0] = (static_cast<char>(register_address) |
               static_cast<char>(RegisterAddress::OP_WRITE));
    data[1] = value;
    spi_select();
    // TODO: ignore response?
...
void NRF24L01::spi_write_register(RegisterAddress register_address,
                                  const char *value, uint8_t length) {
#ifdef _SPI_API_WITHOUT_CS_
    spi_select();
    _spi->write((static_cast<uint8_t>(register_address) |
                 static_cast<uint8_t>(RegisterOperation::OP_WRITE)));
...
uint8_t NRF24L01::spi_read_register(RegisterAddress register_address) {
    ...
    reg = (static_cast<char>(RegisterOperation::OP_READ) |
           static_cast<char>(register_address));
...
void NRF24L01::spi_read_register(RegisterAddress register_address,
                                 uint8_t *value, uint8_t length) {
    static char reg;

    // format register value
    reg = (static_cast<char>(RegisterOperation::OP_READ) |
           (static_cast<char>(register_address) & 0x1F));
