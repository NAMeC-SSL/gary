...
class NRF24L01 {
   public:
    ...
    enum class CRCwidth : uint8_t { NONE = 0, _8bits = 8, _16bits = 16 };
    ...
    void initialize(OperationMode mode, DataRate data_rate,
    ...
    void attach_receive_payload(RxAddressPipe rx_address_pipe, uint8_t *hw_addr,
                                uint8_t payload_size);

    void attach_transmitting_payload(RxAddressPipe rx_address_pipe,
                                     uint8_t *hw_addr, uint8_t payload_size);

    void attach_receive_address_to_pipe(RxAddressPipe rx_address_pipe,
                                        uint8_t *hw_rx_addr);
   ...
   private:
    ...
    void spi_write_register(RegisterAddress register_address, const char *value,
                            uint8_t length);
