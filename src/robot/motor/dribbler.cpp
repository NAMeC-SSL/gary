...
Dribbler::Dribbler_error Dribbler::send_message() {
    RobotToDribbler message = RobotToDribbler_init_zero;
    memset(_dribbler_tx_buffer, 0, sizeof(_dribbler_tx_buffer));

    /* Create a stream that will write to our buffer. */
    pb_ostream_t tx_stream =
        pb_ostream_from_buffer(_dribbler_tx_buffer + 1, RobotToDribbler_size);

    message.command = _current_command;
    message.speed = _current_speed;

    /* Now we are ready to encode the message! */
    bool status = pb_encode(&tx_stream, RobotToDribbler_fields, &message);
    ...
    _spi_mutex->lock();
    _chip_select = 0;
    _spi->write((const char *)_dribbler_tx_buffer, RobotToDribbler_size + 1,
                nullptr, 0);
    _chip_select = 1;
    _spi_mutex->unlock();
...
void Dribbler::set_state(Commands state) { _current_command = state; }

void Dribbler::set_speed(float speed) { _current_speed = speed; }