...
class Brushless_board {
   public:
    ...
    /**
     * @brief Start the communication thread that send messages to the board
     * periodically 10 Hz by default
     *
     */
   ...
   private:
    ...
    uint8_t _brushless_tx_buffer[RobotToBrushless_size + 4 +
                                 1];  // CRC size is 4 bytes
    uint8_t _brushless_rx_buffer[BrushlessToRobot_size + 4 +
                                 1];  // CRC size is 4 bytes + length
};
